...
// WORK STOPPED HERE : make options work for allow no pages folder before to add directpages
...
/**
 * @description
 * Properties that need to be valid path.
 */
const TO_BE_VALID_PATH = [
  'layouts',
  'pages',
  'partials',
  'helpers',
...
const checkValidPaths = (paths) => {
  return new Promise((resolve, reject) => {
    TO_BE_VALID_PATH.forEach(async (key) => {
      if (isInObject(key, paths) && !isValidPath(paths[key])) {
        reject(Error(`[${key}] provided to anah is invalid.`))
      }
    })
    resolve()
  })
...
function checkPathsPresence (paths) {
  return new Promise((resolve, reject) => {
    TO_BE_PROVIDED.forEach(async (key) => {
...
const addPagesRoot = (paths) => {
  const pagesRoot = paths.pages ? addSlashAtEnd(paths.pages) : './'

  filteredOptions.pagesRoot = pagesRoot
