...
// TODO : Return an object with 'raw' propertie with the raw properties, that will be easiest than pass options and filteredOptions to all methods.
/**
 * @description
...
const rewritePaths = async (paths) => {
  Object.keys(paths).forEach(async (key) => {
    ...
    if (TO_REWRITE.includes(key)) {
      filteredOptions[key] = await globify(paths[key], EXTENSIONS[key])
    }
  })
}
