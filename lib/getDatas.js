...
module.exports = async (options) => {
  const glob = await globby(options.datas)
  ...
  for (const file of glob) {
    ...
    const fileContent = await parseFile(file, options, fileName)

    if (fileContent) {
      await processDatas(fileName, options, output, file)
    }
  }
 ...
 */
const processDatas = async (fileName, options, output, file) => {
...
// const resevedFilenameError = (fileName) => {
//   log.error(`A data file can't be named [${fileName}].`)
// }
