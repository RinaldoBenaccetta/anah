...
const processUserHelpers = async (options) => {
  ...
  await Promise.all(
    files.map(async (helper) => {
      ...
      isAlreadyParsed(fileName, options.raw.helpers, options.raw.verbose)
 ...
 */
const isAlreadyParsed = (file, folder, verbose) => {
  alreadyParsed.includes(file)
    ? alreadyParsedWarning(file, folder, verbose)
 ...
 * @param {boolean} verbose The verbose mode.
 */
const alreadyParsedWarning = (file, folder, verbose) => {
  log.warning(
    // eslint-disable-next-line max-len
