...
module.exports = async (folders, options) => {
  const output = await getContent(folders.layouts, options.partials)

  await isEmptyLayoutFolder(output, options.layouts)
...
const isEmptyLayoutFolder = (layouts, layoutFolder) => {
  return new Promise((resolve, reject) => {
    if (Object.keys(layouts).length === 0) {
      reject(
        Error(`Provided [${layoutFolder}] layouts folder contain no template.`)
      )
    }
    resolve()
  })
